...
<div class="transparentsectionbox">
    <h1>Edit Beverage: {{ $drink->name }}</h1>
    <p>Update entry details in the organizational catalog.</p>
</div>

<div class="transparentsectionbox" style="width: 100%; max-width: 600px; margin: 0 auto; text-align: left;">
<div class="glass-card" style="max-width: 600px; margin: 0 auto;">
    <form action="{{ route('drinks.update', $drink) }}" method="POST">
        ...
        <div class="form-group">
            ...
            <input type="text" name="name" id="name" required value="{{ $drink->name }}">
        </div>

        <div class="form-group">
            <label for="description" class="form-label">Flavor Profile / Description</label>
            <textarea name="description" id="description" rows="4">{{ $drink->description }}</textarea>
        </div>
        ...
        <div class="form-group" style="padding: 20px; background: rgba(255,255,255,0.02); border-radius: 16px; border: 1px solid var(--glass-border); flex-direction: row; align-items: center; justify-content: space-between;">
            ...
            <div style="position: relative; display: inline-block; width: 44px; height: 24px;">
                ...
                <input type="checkbox" name="is_available" id="is_available" value="1" {{ $drink->is_available ? 'checked' : '' }} style="opacity: 0; width: 0; height: 0;">
        ...
        <div style="display: flex; gap: 12px; margin-top: 30px;">
            <button type="submit" class="btn btn-blue" style="flex: 1;">Save Changes</button>
